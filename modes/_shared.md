...
6. Resolve a task-aware language context. Write evaluation/dashboard analysis
   in `language.analysis` (legacy `language.output` is read-only compatibility).
   Resolve each job's artifact language from its JD body with `job-language.mjs`;
   tailored CVs, cover letters, and interview materials use that language.
7. Be direct and actionable -- no fluff
8. Use concise, natural prose in the task's selected language. Short sentences,
   action verbs, and no passive voice.
