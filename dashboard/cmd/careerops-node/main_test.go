...
func TestRuntimePathUsesPackagedResourceLayout(t *testing.T) {
	tests := []struct {
		...
		{"windows", "C:/Program Files/CareerOps/careerops-node.exe", "C:/Program Files/CareerOps", "C:/Program Files/CareerOps/runtime/careerops-node-runtime.exe"},
	}
	for _, test := range tests {
		got, err := packagedRuntimePath(test.executable, test.resourceDir, test.goos)
		if err != nil {
...
func TestRuntimeArgsRemovesJitlessOverrides(t *testing.T) {
	...
	got := runtimeArgs([]string{"--no-jitless", "--jitless=false", "intake.mjs"})
...
func TestRuntimeEnvironmentPinsNodeOptions(t *testing.T) {
	got := runtimeEnvironment([]string{"PATH=/bin", "NODE_OPTIONS=--jitless=false"})
