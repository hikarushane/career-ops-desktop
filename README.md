...
> **Windows 安裝檔自 0.5.2 起提供**（0.5.0 與 0.5.1 只有 macOS 版）。Windows 版是 NSIS 安裝檔，尚未做 Authenticode 簽章：第一次執行時 SmartScreen 會顯示「Windows 已保護您的電腦」，按「其他資訊」→「仍要執行」即可。部分防毒軟體可能將未簽章的安裝檔誤判為可疑（已知：Avira），請加入例外或回報誤判。
...
| **AI Provider** | 透過 provider abstraction 偵測並使用本機 AI CLI，不把 App 綁死在單一 agent。0.5.0 驗證過的 provider 是 **Claude Code**、**Codex** 與 **Antigravity（agy）**；三者都要先在系統上安裝並登入。第一次啟動的 Install 按鈕會開一個終端機視窗執行該 provider 的官方安裝指令，登入也在那裡完成。 |
