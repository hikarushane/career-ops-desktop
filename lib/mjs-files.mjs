...
/**
 * Directories excluded from the walk BY PATH below the walk root.
 *
 * Tauri build outputs of the Desktop fork: `desktop/src-tauri/binaries/` holds
 * the packaged workspace seed — a copy of every runtime `.mjs` in this
 * repository — and `desktop/src-tauri/target/` holds cargo's build tree, which
 * carries that copy again per profile. Walking them grades each runtime file
 * two to four times over, under paths no guard or exemption can name, and the
 * count depends on which builds a checkout has run. Scoped to these paths so a
 * directory that merely shares a name stays under enforcement.
 */
export const SKIP_PATHS = new Set(['desktop/src-tauri/binaries', 'desktop/src-tauri/target']);
