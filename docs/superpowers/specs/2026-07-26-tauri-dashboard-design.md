...
3. **Toggling the view mode does not re-sort.** `case "v"` (pipeline.go:326-332) flips `viewMode` between grouped and flat but never calls `applyFilterAndSort`, unlike the tab and sort handlers beside it. Grouped mode applies an extra ordering pass by status priority, so switching into it leaves the previous order on screen under a "grouped" label until some other keypress triggers a re-sort. Found while verifying the desktop app's filter parity against the TUI.
