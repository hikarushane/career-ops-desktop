...
| --- | --- |
| **Onboarding** | Import background documents, configure AI, choose the analysis language, state your job preferences, and get ready without editing YAML by hand. The AI writes `cv.md`, `config/profile.yml`, `modes/_profile.md`, and `portals.yml` from your documents and preferences; you preview them before applying and can regenerate with feedback. `portals.yml` does not inherit the template's company list: the AI fills in the job boards people actually use in your target countries (for example 104, 1111, CakeResume, 就業通, and LinkedIn in Taiwan; StepStone, Indeed, and LinkedIn in Germany, plus field boards such as get-in-engineering and ingenieur.de for engineers) and 15 to 40 matching employers. |
| **AI provider** | Detect and use local AI CLIs through a provider abstraction instead of hard-coding one agent. The providers verified for 0.5.0 are **Claude Code**, **Codex** and **Antigravity (agy)**; each must be installed and signed in on your machine. Onboarding's Install button opens a terminal window with the provider's official installer; sign in there. |
| **Job evaluation** | Paste a job URL and the app fetches the posting itself first (LinkedIn through its public guest endpoint; pages that render the posting with JavaScript, such as StepStone, through the JSON-LD job data embedded in the page; other sites over plain HTTP). Only when a site blocks that (Indeed, for example) does it ask you to paste the job description. You can also paste a full JD directly. The AI evaluation starts only after the text is captured, and finishing opens the new report's card. |
| **Job discovery** | Run the scanner from the UI; the **Process pending jobs** card on Home shows how many entries in `data/pipeline.md` are pending or need attention and starts a batch run that chains turn after turn until the inbox is empty. |
| **Task center** | Evaluations, scans, batch runs, and interview tasks keep running when you switch screens; the header task chip (with an amber breathing halo while running) shows running / done / failed and takes you back to that task's activity log. While a scan or batch runs, Home shows its progress, and the progress screen's top-left Back leaves without cancelling. The log shows which files the AI is actually reading and writing, and success means a report was produced, not merely a zero exit code. |
| **Applications** | Browse the existing CareerOps pipeline, reports, statuses, PDFs, and progress in a native interface. Drag a Kanban card onto a status column to change its status; the report panel has a status select at the top, a draggable left edge, and the table view spans the full width until a row is picked. On a report card, **Generate CV** produces the tailored CV PDF for that report straight away; **Generate cover letter** first asks you four questions (why this company, what problem you would solve, your approach, tone) and then writes the cover letter PDF. Once the files exist the buttons become **View CV** / **View cover letter**, which reveal the file in Finder or File Explorer; **View job description** opens the raw capture from `jds/` inside the panel. |
| **Interview** | Prep plan, Practice and Debrief are conversations with the AI: a short intake form first (the interview date and time, for example), then follow-up questions and additions after the first turn, with the conversation kept between visits. Plans, practice transcripts and debriefs the AI writes under `interview-prep/` are listed under each reply and under the company's card on the Interview screen, and open in the side panel. |
| **Language system** | Choose the language used to read analysis while CVs, cover letters, and interview materials follow each job description's language. The interface itself switches between English and Traditional Chinese from the globe tab in Settings. |
| **Help and settings** | Manage profile, sources, AI provider, language, and help content inside the app. Settings → My Background's Regenerate profile shows the same import screen as onboarding before regenerating. Settings → Job Search is the same preferences form as onboarding; after editing it, the AI can rewrite only `config/profile.yml`, `modes/_profile.md` and `portals.yml` (never `cv.md`), previewed before it is applied. In Settings → AI the Model field is a dropdown: agy is read from `agy models`, while claude and codex candidates are probed with one tiny live call each to see which your account can use (cached for 24 hours; Refresh re-probes). Fast mode is available only for Claude Opus models, and Effort does not apply to agy. The full guide in Help switches between Chinese and English. |
